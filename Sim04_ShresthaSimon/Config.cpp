...
void Config::readConfigFile(string configFilePath) {
    ...
    for(auto pnt = iterator.begin(); pnt != iterator.end(); pnt++){
        ...
        if(leftSide == "Version/Phase"){
            ...
            try{
                memoryType = leftSide;
                if(memoryType.find("kbytes") != string::npos){
                    memory = stoi(rightSide);
                }else if(memoryType.find("Mbytes") != string::npos){
                    memory = stoi(rightSide) * 1000;
                }else if(memoryType.find("Gbytes") != string::npos){
                    memory = stoi(rightSide) * 1000000;
                }
            }
            catch (int e) {
            ...
            try{
                memoryType = leftSide;
                if(memoryType.find("kbytes") != string::npos){
                    memBlockSize = stoi(rightSide);
                }else if(memoryType.find("Mbytes") != string::npos){
                    memBlockSize= stoi(rightSide) * 1000;
                }else if(memoryType.find("Gbytes") != string::npos){
                    memBlockSize = stoi(rightSide) * 1000000;
                }
            }
            catch (int e) {
            ...
            try{
                schedulingAlgorithm = rightSide;
            }
            catch (int e){
                cerr << "Error! Incorrect Scheduling Algo format." << endl;
                exit(0);
            }
        }else if(leftSide == "Processor Quantum Number"){
            try{
                processorQNUM = stoi(rightSide);
            }
            catch (int e){
...
//
const string &Config::getSchedulingAlgorithm() const {
...
//
int Config::getMemBlockSize() const{
    return memBlockSize;
}
//
